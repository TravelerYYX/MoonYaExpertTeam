...
# Define split points (start, end, target_file) - CONTIGUOUS, no gaps
$splits = @(
    # CSS files (lines 21-4775)
...
foreach ($split in $sortedSplits) {
    ...
    if ($start -ne ($prevEnd + 1)) {
        Write-Host "CONTIGUITY ERROR: prevEnd=$prevEnd, start=$start (expected $($prevEnd + 1))"
        $ok = $false
    }
    $prevEnd = $end
}
if ($ok) {
    Write-Host "All splits are contiguous."
}
Write-Host "Last split ends at: $prevEnd (expected: $totalLines)"

# Now write the files
foreach ($split in $splits) {
    $start = $split.Start - 1  # Convert to 0-based
    $end = $split.End - 1
    $count = $end - $start + 1
    ...
    Write-Host "Wrote: $($split.Path) ($count lines)"
}
...
Write-Host "Total lines written: $totalWritten (expected: $totalLines)"

# Verify content of first and last lines of each file
Write-Host "`n=== Boundary check ==="
foreach ($split in $splits) {
    $targetPath = Join-Path $base $split.Path
    $fileLines = [System.IO.File]::ReadAllLines($targetPath)
    $first = $fileLines[0]
    $last = $fileLines[-1]
    Write-Host "$($split.Path): first=[$first], last=[$last]"
}
